...
CREATE TABLE Roles (
    ...
    -- Rollenname darf nur einmal existieren (UNIQUE)
...
CREATE TABLE Cities (
    ...
    -- nur unique Kombinationen von PLZ/PostCode und Stadt/City erlaubt
...
CREATE TABLE Users (
    ...
    -- Nur 'Active' und 'Banned' sind als UserState erlaubt (CHECK)
    UserState VARCHAR(30) NOT NULL DEFAULT 'Active'
        CHECK (UserState IN ('Active', 'Banned')),
...
CREATE TABLE Authors (
    ...
    -- Autoren dürfen nur einmalig existieren
...
CREATE TABLE Publishers (
    ...
    -- Verlage dürfen nur einmalig existieren
    Publisher VARCHAR(200) NOT NULL UNIQUE
);
...
CREATE TABLE Languages (
    ...
    -- Sprachen dürfen nur einmalig existieren
...
CREATE TABLE Genres (
    ...
    -- Genres dürfen nur einmalig existieren
...
CREATE TABLE Books (
    ...
    -- ISBN darf nur einmalig existieren
    ISBN VARCHAR(17) UNIQUE,
    PublisherId INTEGER,
    LanguageId INTEGER,
    MaxBorrowTime INTEGER NOT NULL
        CHECK (MaxBorrowTime > 0),
...
CREATE TABLE BorrowRequests (
    ...
    -- Nur die States Pending, Accepted, Rejected, Cancelled sind erlaubt: 
    BorrowRequestState VARCHAR(30) NOT NULL
        CHECK (
            BorrowRequestState IN (
                'Pending',
                'Accepted',
                'Rejected',
                'Cancelled'
    ...
    CHECK (PickupWindowEnd > PickupWindowStart),

    -- Abgabedatum darf nicht vor Ausleihdatum liegen 
    CHECK (RequestedUntilTime >= PickupWindowEnd)
);
...
CREATE TABLE Borrows (
    ...
    -- Nur die States Active & Returned sind erlaubt: 
    BorrowState VARCHAR(30) NOT NULL
        CHECK (BorrowState IN ('Active', 'Returned')),
    ...
    -- Enddatum darf nicht vor Startdatum liegen
    CHECK (PickupWindowEnd > PickupWindowStart),
    CHECK (ReturnWindowEnd > ReturnWindowStart),
    CHECK (ReturnWindowStart > PickupWindowEnd)
);
...
-- ================================================================================
-- Indizes setzen zur Performancesteigerung auf Foreign-Key-Spalten 
